...
def polynomialsEvalRep(field, omega, n):
    ...
    class PolynomialEvalRep(object):
        ...
        def _reciprocal(self):
            ...
            if k == 1:
                return PolynomialEvalRep.from_coeffs(
                    _poly_coeff([field(1) / field(poly.coefficients[0])])
                )
            else:
                q = PolynomialEvalRep.from_coeffs(
                    _poly_coeff(poly.coefficients[k // 2 :])
                )._reciprocal()
        ...
        def __divmod__(self, other):
            ...
            if deg > 2 * other_deg:
                ...
                x_pow_2n = PolynomialEvalRep.from_coeffs(
                    _poly_coeff([field(0)] * (2 * other_deg) + [field(1)])
                )
